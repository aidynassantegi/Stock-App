...
//  Stock App
//
//  Created by Aidyn Assan on 27.07.2022.
//

import UIKit
import Charts

protocol StockDetailsViewInput: AnyObject {
	func fetchIntervals(with intervals: [String])
	func fetchStockData(with stock: Stock)
}

protocol StockDetailsViewOutput: AnyObject {
	func didLoadView()
	func didSelectIntervalCell(with interval: String)
	func didTapBuyButton(for stock: Stock)
}

class StockDetailsView: UIViewController {
	
	var output: StockDetailsViewOutput?
	...
	func setView(with stock: Stock) {
		title = stock.companyName
		priceLabel.text = "$\(stock.price)"
		changeLabel.text = "\(stock.changes)%"
		buyButton.setTitle("Buy for \(stock.price)", for: .normal)
		
//		chart.set(with: stock)
...
extension StockDetailsView: StockDetailsViewInput {
	func fetchIntervals(with intervals: [String]) {
...
extension StockDetailsView: UICollectionViewDataSource, UICollectionViewDelegate {
